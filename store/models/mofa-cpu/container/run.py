"""MOFA (CPU-only) model container entrypoint.

CPU build of the MOFA+ container — identical to the ``mofa`` container but using
the ``mofa-cpu`` model slug so its parameters resolve from the CPU config.
...
class MOFAModel(ModelFactory):
    ...
    def __init__(
        ...
        Raises:
            ValueError: If 'mofa-cpu' configuration is not found in the model parameters.
        """
...
def main() -> None:
    ...
    config = build_model_config("mofa-cpu", job_spec, OUTPUT_DIR)
