...
Hot-path module. Per ADR §8 the mvd kernel must not import MLflow, Optuna, GC,
...
final artifact bundle directory. Every step is idempotent, every step writes
its intent to the journal *before* the side effect, and every step refuses to
...
* moving an owned-failed staging directory to ``store/quarantine/`` in the
  failure branch.
