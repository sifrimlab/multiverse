...
def _seed_legacy(root: Path) -> None:
    ...
    conn = sqlite3.connect(str(root / "mvexp_state.db"))
...
def test_dry_run_reports_plan_without_moving(tmp_path, capsys):
    ...
    assert not dst.exists() or not (dst / "mvexp_state.db").exists()
